...
void afficher_matrice(Pixel** matrice, int L, int C)
{
    ...
    for (int x = 0; x < L; x++) {
        ...
        for (int y = 0; y < C; y++) {
            printf("\x1B[48;2;%d;%d;%dm  \x1B[0m", matrice[x][y].r, matrice[x][y].g, matrice[x][y].b);
        }
        printf("\n");
    }
}
...
char* matrice_to_string(Pixel** matrice, int nb_lignes, int nb_colonnes) {
    ...
    for (int i = 0; i < nb_lignes; i++) {
        memcpy(&buffer[i * nb_colonnes], matrice[i], nb_colonnes * sizeof(struct Pixel));
    }
...
struct Pixel** string_to_matrice(char* buffer, int nb_lignes, int nb_colonnes) {
    ...
    for (int i = 0; i < nb_lignes; i++) {
        for (int j = 0; j < nb_colonnes; j++) {
            memcpy(&matrice[i][j], &buffer[(i * nb_colonnes + j) * sizeof(struct Pixel)], sizeof(struct Pixel));
        }
    }
